...
PPUObj::PPUObj() {
  if (!SDL_CreateWindowAndRenderer("YAGBA", 160, 144, 0, &win, &renderer)) {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error",
                             "Window could not be created", NULL);
    shouldExit = true;
    return;
  }

  SDL_SetWindowSize(win, 480, 432);
  SDL_SetRenderLogicalPresentation(renderer, 160, 144,
                                   SDL_LOGICAL_PRESENTATION_LETTERBOX);

  SDL_SetWindowResizable(win, true);

  renderTarget = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32,
...
int COLORS[] = {0xff, 0xff, 0xff, 0xaa, 0xaa, 0xaa,
                0x55, 0x55, 0x55, 0x00, 0x00, 0x00};

/**
 * @brief Calculates the pixel data for the background and window layers for a
 * given scanline.
 *
 * This function renders the background and window layers for the specified
 * `row` (scanline). It reads tile data and tile maps from VRAM based on LCDC
 * register settings, SCX/SCY scroll registers, and WX/WY window position
 * registers. Colors are determined using the BGP palette register (0xFF47). The
 * results are stored in the `background` and `window` pixel arrays. It also
 * renders sprites that are visible on this scanline.
 *
 * @param row The current scanline number (LY register value, 0-143 for visible
 * lines).
 */
void PPUObj::calculateMaps(uint8_t row) {
  ...
  uint16_t windowTileMapArea =
      (LCDC & 0x40) ? 0x9C00 : 0x9800; // LCDC Bit 6 for Window
  bool signedTileAddressing =
      !(LCDC & 0x10); // LCDC Bit 4: 0 = 0x8800 method, 1 = 0x8000 method
  uint16_t tileDataBaseAddress = (LCDC & 0x10) ? 0x8000 : 0x8800;

  uint8_t palette = memory->get(0xff47);

  for (int j = 0; j < 256;
       j++) { // Iterate across the 256-pixel wide virtual map
    // Background Pixel
    uint8_t offY_bg = row + SCY;
    uint8_t offX_bg = j + SCX;
    int colour_bg = 0;

    uint8_t tile_index_bg =
        memory->get(bgTileMapArea + ((offY_bg / 8 * 32) + (offX_bg / 8)));
    uint16_t tile_addr_bg;

    if (signedTileAddressing) { // 0x8800 method (signed index)
      tile_addr_bg =
          tileDataBaseAddress + 0x800 + (((int8_t)tile_index_bg) * 0x10);
    } else { // 0x8000 method (unsigned index)
      tile_addr_bg = tileDataBaseAddress + (tile_index_bg * 0x10);
    }
    colour_bg =
        (memory->get(tile_addr_bg + (offY_bg % 8 * 2)) >> (7 - (offX_bg % 8)) &
         0x1) +
        ((memory->get(tile_addr_bg + (offY_bg % 8 * 2) + 1) >>
              (7 - (offX_bg % 8)) &
          0x1) *
         2);
    ...
    if (windowEnabled && WX <= j && WY <= row) {
      // Check if current pixel (j) is within window's horizontal range on
      // screen (0-159) And current row is within window's vertical range on
      // screen (0-143) This check is implicitly handled by how drawFrame later
      // crops from the 256x256 buffers. The WX <= j and WY <= row determines if
      // the window *starts* covering pixels.

      uint8_t offY_win = row - WY;
      uint8_t offX_win = j - WX;
      int colour_win = 0;

      uint8_t tile_index_win = memory->get(
          windowTileMapArea + ((offY_win / 8 * 32) + (offX_win / 8)));
      uint16_t tile_addr_win;

      if (signedTileAddressing) { // 0x8800 method
        tile_addr_win =
            tileDataBaseAddress + 0x800 + (((int8_t)tile_index_win) * 0x10);
      } else { // 0x8000 method
        tile_addr_win = tileDataBaseAddress + (tile_index_win * 0x10);
      }

      colour_win = (memory->get(tile_addr_win + (offY_win % 8 * 2)) >>
                        (7 - (offX_win % 8)) &
                    0x1) +
                   ((memory->get(tile_addr_win + (offY_win % 8 * 2) + 1) >>
                         (7 - (offX_win % 8)) &
                     0x1) *
                    2);
      ...
      // If window not active here, ensure window buffer is transparent for this
      // pixel
      window[(row * 256 * 4) + (j * 4) + 3] = 0x00;
    }
  }

  // Sprite rendering (largely unchanged for this specific fix, but see
  // conceptual points later)
      ...
      if (row >= (y - 16) && row <= ((y - 16) + height)) {
        ...
        for (int u = 0; u < height; u++) {
          for (int v = 0; v < 8; v++) {
            ...
            case 0x00:
              colour =
                  (memory->get(0x8000 + (t * 0x10) + (u * 2)) >> (7 - v) &
                   0x1) +
                  (memory->get(0x8000 + (t * 0x10) + (u * 2) + 1) >> (7 - v) &
                   0x1) *
                      2;
              break;
            case 0x20:
              colour =
                  (memory->get(0x8000 + (t * 0x10) + (u * 2)) >> v & 0x1) +
                  (memory->get(0x8000 + (t * 0x10) + (u * 2) + 1) >> v & 0x1) *
                      2;
              break;
            case 0x40:
              colour =
                  (memory->get(0x8000 + (t * 0x10) + ((height - u - 1) * 2)) >>
                       (7 - v) &
                   0x1) +
                  (memory->get(0x8000 + (t * 0x10) + ((height - u - 1) * 2) +
                               1) >>
            ...
            uint8_t colorfrompal =
                (memory->get(f >> 4 & 1 ? 0xff49 : 0xff48) >> (2 * colour)) & 3;

            if (colour && ((y + u) >= 16 && (y + u) <= 0xff) &&
                ((x + v) >= 8 && (x + v) <= 0xff)) {
              sprites[((y + u - 16) * 256 * 4) + ((x + v - 8) * 4)] =
                  COLORS[colorfrompal * 3];
              sprites[((y + u - 16) * 256 * 4) + ((x + v - 8) * 4) + 1] =
                  COLORS[colorfrompal * 3 + 1];
              sprites[((y + u - 16) * 256 * 4) + ((x + v - 8) * 4) + 2] =
                  COLORS[colorfrompal * 3 + 2];
 ...
 * implementation might overlay unconditionally if sprite pixel is not
 * transparent). The resulting framebuffer is then updated to an SDL texture and
...
void PPUObj::drawFrame() {
  for (int row = 0; row < 144; row++) {
    for (int col = 0; col < 160; col++) {
      ...
      framebuffer[(row * 160 * 4) + (col * 4) + 1] =
          background[yoff + xoff + 1];
      framebuffer[(row * 160 * 4) + (col * 4) + 2] =
          background[yoff + xoff + 2];
      framebuffer[(row * 160 * 4) + (col * 4) + 3] =
          background[yoff + xoff + 3];
  ...
  SDL_UpdateTexture(renderTarget, NULL, framebuffer.data(),
                    160 * sizeof(unsigned char) * 4);
